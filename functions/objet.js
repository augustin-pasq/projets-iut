class CalculDistance {
    ...
    calculDistanceTrajet(fileName) {
        ...
        let distance = 0;
        for (let i = 0; i < data.length - 1; i += 1) distance += CalculDistance.prototype.calculDistance2PointsGPS(data[i].latitude, data[i].longitude, data[i + 1].latitude, data[i + 1].longitude);
